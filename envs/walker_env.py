...
import numpy as np
import Box2D
from Box2D.b2 import world, polygonShape, revoluteJointDef, fixtureDef, edgeShape
import pygame
...
class WalkerEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.max_steps = 1000
        self.motor_speed_multiplier = 4.0
        self.fall_angle_threshold = 1.0
        ...
        self.ground_body = self.world.CreateStaticBody(
            position=(0, 0),
            fixtures=fixtureDef(
                shape=edgeShape(vertices=[(-1000, 0), (1000, 0)]),
                friction=0.8 
            )
        )
    ...
    def step(self, action):
        # 1. Appliquer les actions
        self.joint_hanche_g.motorSpeed = float(action[0]) * self.motor_speed_multiplier
        self.joint_genou_g.motorSpeed = float(action[1]) * self.motor_speed_multiplier
        self.joint_hanche_d.motorSpeed = float(action[2]) * self.motor_speed_multiplier
        self.joint_genou_d.motorSpeed = float(action[3]) * self.motor_speed_multiplier

        # 2. Avancer la physique
        self.world.Step(self.dt, 6, 2)
        self.steps += 1 

        # 3. Récupérer l'observation
        obs = self._get_observation()

        # 4. Vérifier les conditions de fin
        terminated = bool(
            abs(self.torse.angle) > self.fall_angle_threshold or 
            self.torse.position.x < self.start_x - 0.5
        )
    ...
    def render(self):
        ...
        def draw_polygon(polygon, body, color):
            vertices = [(body.transform * v) for v in polygon.vertices]
            pygame_verts = [
                (int((v[0] - cam_x) * self.ppm + self.window_size[0] / 2),
                 int(self.window_size[1] - (v[1] * self.ppm) - 50))
                for v in vertices
            ]
            pygame.draw.polygon(self.screen, color, pygame_verts)
            pygame.draw.polygon(self.screen, (0, 0, 0), pygame_verts, 2)
        ...
        # Dessiner le sol
        sol_y = int(self.window_size[1] - 50)
        pygame.draw.rect(self.screen, (34, 139, 34), (0, sol_y, self.window_size[0], self.window_size[1] - sol_y))
        pygame.draw.line(self.screen, (0, 0, 0), (0, sol_y), (self.window_size[0], sol_y), 3)
    ...
    def _create_robot(self, x_torse, y_torse):
        # Filtre pour éviter l'auto-collision des membres du robot
        robot_filter = Box2D.b2.filter(groupIndex=-1)

        hull_fixture = fixtureDef(shape=polygonShape(box=(0.25, 0.15)), density=5.0, friction=0.1, filter=robot_filter)
        cuisse_fixture = fixtureDef(shape=polygonShape(box=(0.06, 0.2)), density=1.0, friction=0.8, filter=robot_filter)
        tibia_fixture = fixtureDef(shape=polygonShape(box=(0.05, 0.2)), density=1.0, friction=0.8, filter=robot_filter)
    ...
    def _compute_reward(self, action, terminated):
        x = float(self.torse.position.x)
        velocity_x = (x - self.prev_x) / self.dt
        self.prev_x = x

        # Étape 1 : Vitesse
        reward = velocity_x
        
        # Étape 2 : Pénalité énergétique
        reward -= 0.003 * float(np.sum(np.square(action)))

        # Étape 3 : Survie et Chute
        if terminated:
            reward -= 100.0
        else:
            reward += 0.001
...
if __name__ == "__main__":
    print("Test de l'environnement BladeWalker optimisé...")
    ...
    while running:
        ...
        if terminated or truncated:
            print(f"Épisode {episodes} terminé. Récompense finale : {reward:.2f}")
            obs, info = env.reset()
            episodes += 1
